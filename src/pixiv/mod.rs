...
pub async fn auth_with_credential(
    ...
    if let Some(at) = &cred.access_token {
        ...
        // Persist the freshly obtained tokens so they survive this job.
...
/// Read tokens from the `PixivApi` and persist them to the database.
async fn persist_tokens(
    ...
    query::pixiv_credential::update_token(db, credential_id, refresh_to_save, new_access.as_deref())
        .await
        .map_err(|e| format!("Failed to persist refreshed tokens: {}", e))?;

    // Ensure status is ACTIVE after successful auth.
