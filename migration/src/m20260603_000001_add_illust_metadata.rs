...
impl MigrationTrait for Migration {
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        let db = manager.get_connection();
        let _ = db.execute_unprepared("ALTER TABLE images DROP COLUMN illust_type")
            .await;
        let _ = db.execute_unprepared("ALTER TABLE images DROP COLUMN x_restrict")
            .await;
        let _ = db.execute_unprepared("ALTER TABLE images DROP COLUMN illust_ai_type")
            .await;
        Ok(())
    }
}
