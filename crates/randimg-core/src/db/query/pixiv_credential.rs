...
/// Update tokens and user_id after a successful refresh.
///
/// `pixiv_user_id` is updated so that subsequent `set_auth()` calls use the
/// correct numeric ID returned by Pixiv's OAuth endpoint, rather than whatever
/// stale or placeholder value was stored when the credential was first created.
pub async fn update_token(
    db: &DatabaseConnection,
    id: i32,
    refresh_token: &str,
    access_token: Option<&str>,
    pixiv_user_id: Option<u64>,
) -> Result<(), DbErr> {
    let Some(model) = PixivCredential::find_by_id(id).one(db).await? else {
        return Ok(());
    };
    ...
    if let Some(uid) = pixiv_user_id {
        active.pixiv_user_id = Set(uid.to_string());
    }
