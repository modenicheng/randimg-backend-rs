...
pub async fn find_crawl_ids_by_type(
    ...
    let sql = "SELECT id FROM Jobs WHERE job_type = 'randimg_core::task_queue::jobs::CrawlJob' AND json_extract(job, '$.crawl_type') = ?";
...
pub async fn find_crawl_ids_by_type(
    ...
    let sql = "SELECT id FROM apalis.jobs WHERE job_type = 'randimg_core::task_queue::jobs::CrawlJob' AND (job::json->>'crawl_type')::int = $1";
    let stmt = sea_orm::Statement::from_sql_and_values(
        db.get_database_backend(),
        sql,
        [crawl_type.into()],
