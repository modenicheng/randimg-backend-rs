...
async fn test_tag_find_all() {
    let db = setup_db().await;
    randimg_core::db::query::tag::find_or_create(&db, "a", None)
        .await
        .unwrap();
    randimg_core::db::query::tag::find_or_create(&db, "b", None)
        .await
        .unwrap();
    randimg_core::db::query::tag::find_or_create(&db, "c", None)
        .await
        .unwrap();

    let all = randimg_core::db::query::tag::find_all(&db, None, None)
        .await
        .unwrap();
    assert_eq!(all.len(), 3);

    let limited = randimg_core::db::query::tag::find_all(&db, Some(2), None)
        .await
        .unwrap();
    assert_eq!(limited.len(), 2);

    // SQLite requires LIMIT when using OFFSET
    let offset = randimg_core::db::query::tag::find_all(&db, Some(10), Some(1))
        .await
        .unwrap();
...
async fn test_author_find_or_create_by_name() {
    let db = setup_db().await;
    let a1 = randimg_core::db::query::author::find_or_create(&db, "SameName", Some("pixiv"), None)
        .await
        .unwrap();
    let a2 =
        randimg_core::db::query::author::find_or_create(&db, "SameName", Some("twitter"), None)
            .await
            .unwrap();
    assert_eq!(a1.id, a2.id);
}
...
async fn test_author_count() {
    let db = setup_db().await;
    assert_eq!(
        randimg_core::db::query::author::count(&db).await.unwrap(),
        0
    );
    ...
    randimg_core::db::query::author::find_or_create(&db, "A1", Some("p"), Some("1"))
        .await
        .unwrap();
    randimg_core::db::query::author::find_or_create(&db, "A2", Some("p"), Some("2"))
        .await
        .unwrap();
    assert_eq!(
        randimg_core::db::query::author::count(&db).await.unwrap(),
...
async fn test_author_find_all_with_pagination() {
    ...
    let all = randimg_core::db::query::author::find_all(&db, None, None)
        .await
        .unwrap();
    assert_eq!(all.len(), 5);

    let page = randimg_core::db::query::author::find_all(&db, Some(2), Some(1))
        .await
        .unwrap();
...
async fn test_author_find_by_id() {
    let db = setup_db().await;
    let author =
        randimg_core::db::query::author::find_or_create(&db, "FindMe", Some("p"), Some("fid"))
            .await
            .unwrap();

    let found = randimg_core::db::query::author::find_by_id(&db, author.id)
        .await
        .unwrap();
    assert!(found.is_some());
    assert_eq!(found.unwrap().name, "FindMe");

    let not_found = randimg_core::db::query::author::find_by_id(&db, 99999)
        .await
        .unwrap();
...
async fn test_crawler_find_all() {
    let db = setup_db().await;
    randimg_core::db::query::crawler::create(&db, "c1", 0, None, None, None, None)
        .await
        .unwrap();
    randimg_core::db::query::crawler::create(&db, "c2", 1, Some("u"), None, None, None)
        .await
        .unwrap();

    let all = randimg_core::db::query::crawler::find_all(&db)
        .await
        .unwrap();
    assert_eq!(all.len(), 2);
}
...
async fn test_crawler_mark_running() {
    let db = setup_db().await;
    let c = randimg_core::db::query::crawler::create(&db, "run", 1, Some("u"), None, None, None)
        .await
        .unwrap();
    assert_eq!(c.status, 0);
    assert!(c.start_time.is_none());

    randimg_core::db::query::crawler::mark_running(&db, c.id)
        .await
        .unwrap();
    let updated = randimg_core::db::query::crawler::find_by_id(&db, c.id)
        .await
        .unwrap()
        .unwrap();
...
async fn test_crawler_mark_completed() {
    let db = setup_db().await;
    let c = randimg_core::db::query::crawler::create(&db, "comp", 1, Some("u"), None, None, None)
        .await
        .unwrap();

    randimg_core::db::query::crawler::mark_completed(&db, c.id, 10)
        .await
        .unwrap();
    let updated = randimg_core::db::query::crawler::find_by_id(&db, c.id)
        .await
...
async fn test_crawler_mark_failed() {
    let db = setup_db().await;
    let c = randimg_core::db::query::crawler::create(&db, "fail", 1, Some("u"), None, None, None)
        .await
        .unwrap();

    randimg_core::db::query::crawler::mark_failed(&db, c.id)
        .await
        .unwrap();
    let updated = randimg_core::db::query::crawler::find_by_id(&db, c.id)
        .await
...
async fn test_crawler_find_by_id_not_found() {
    let db = setup_db().await;
    let result = randimg_core::db::query::crawler::find_by_id(&db, 99999)
        .await
        .unwrap();
    assert!(result.is_none());
}
...
async fn test_pixiv_credential_find_all() {
    let db = setup_db().await;
    randimg_core::db::query::pixiv_credential::create(&db, "u1", "rt1", None)
        .await
        .unwrap();
    randimg_core::db::query::pixiv_credential::create(&db, "u2", "rt2", None)
        .await
        .unwrap();

    let all = randimg_core::db::query::pixiv_credential::find_all(&db)
        .await
        .unwrap();
    assert_eq!(all.len(), 2);
}
...
async fn test_pixiv_credential_update_partial() {
    let db = setup_db().await;
    let cred = randimg_core::db::query::pixiv_credential::create(&db, "u1", "rt", None)
        .await
        .unwrap();

    // Update only status
    let updated =
        randimg_core::db::query::pixiv_credential::update(&db, cred.id, None, None, Some(1), None)
            .await
            .unwrap();
    assert!(updated.is_some());
    let updated = updated.unwrap();
...
async fn test_pixiv_credential_update_token() {
    let db = setup_db().await;
    let cred = randimg_core::db::query::pixiv_credential::create(&db, "u1", "old_rt", None)
        .await
        .unwrap();

    randimg_core::db::query::pixiv_credential::update_token(
        &db,
        cred.id,
        "new_rt",
        Some("new_at"),
    ...
    let updated = randimg_core::db::query::pixiv_credential::find_by_id(&db, cred.id)
        .await
        .unwrap()
        .unwrap();
...
async fn test_pixiv_credential_update_status() {
    ...
    randimg_core::db::query::pixiv_credential::update_status(&db, cred.id, 2)
        .await
        .unwrap();
    let updated = randimg_core::db::query::pixiv_credential::find_by_id(&db, cred.id)
        .await
...
async fn test_pixiv_credential_touch_last_used() {
    ...
    randimg_core::db::query::pixiv_credential::touch_last_used(&db, cred.id)
        .await
        .unwrap();
    let updated = randimg_core::db::query::pixiv_credential::find_by_id(&db, cred.id)
        .await
...
async fn test_pixiv_credential_delete() {
    ...
    let deleted = randimg_core::db::query::pixiv_credential::delete(&db, cred.id)
        .await
        .unwrap();
    assert!(deleted);
...
async fn test_pixiv_credential_delete_not_found() {
    let db = setup_db().await;
    let deleted = randimg_core::db::query::pixiv_credential::delete(&db, 99999)
        .await
        .unwrap();
    assert!(!deleted);
}
...
async fn test_pixiv_credential_find_one_active_random_none_when_empty() {
    let db = setup_db().await;
    let result = randimg_core::db::query::pixiv_credential::find_one_active_random(&db)
        .await
        .unwrap();
    assert!(result.is_none());
}
...
async fn test_pixiv_credential_find_one_active_random_returns_active() {
    ...
    let expired = randimg_core::db::query::pixiv_credential::create(&db, "u2", "rt2", None)
        .await
        .unwrap();
    randimg_core::db::query::pixiv_credential::update_status(&db, expired.id, 1)
        .await
        .unwrap();
...
async fn test_pixiv_credential_update_not_found() {
    let db = setup_db().await;
    let result =
        randimg_core::db::query::pixiv_credential::update(&db, 99999, Some("rt"), None, None, None)
            .await
            .unwrap();
    assert!(result.is_none());
}
...
async fn test_image_create_and_find_by_id() {
    ...
    let author =
        randimg_core::db::query::author::find_or_create(&db, "Artist", Some("p"), Some("1"))
            .await
            .unwrap();
    ...
    randimg_core::db::query::image::update_fields(&db, img.id, update_data)
        .await
        .unwrap();

    let found = randimg_core::db::query::image::find_by_id(&db, img.id, false, &config)
        .await
        .unwrap();
    assert!(found.is_some());
    let found = found.unwrap();
...
async fn test_image_find_by_id_not_found() {
    let db = setup_db().await;
    let config = make_test_config();
    let found = randimg_core::db::query::image::find_by_id(&db, 99999, false, &config)
        .await
        .unwrap();
    assert!(found.is_none());
}
...
async fn test_image_find_by_id_inaccessible_hidden_for_non_admin() {
    let db = setup_db().await;
    let config = make_test_config();
    let author = randimg_core::db::query::author::find_or_create(&db, "A", Some("p"), Some("1"))
        .await
        .unwrap();
    ...
    // Admin should see it
    let found = randimg_core::db::query::image::find_by_id(&db, img.id, true, &config)
        .await
        .unwrap();
    assert!(found.is_some());
}
...
async fn test_image_update_fields() {
    ...
    let updated = randimg_core::db::query::image::update_fields(&db, img.id, data)
        .await
        .unwrap();
    assert!(updated.is_some());
    let updated = updated.unwrap();
...
async fn test_image_find_unprocessed() {
    ...
    randimg_core::db::query::image::update_fields(&db, processed.id, update_data)
        .await
        .unwrap();

    let unprocessed = randimg_core::db::query::image::find_unprocessed(&db)
        .await
        .unwrap();
...
async fn test_image_count_accessible() {
    let db = setup_db().await;
    assert_eq!(
        randimg_core::db::query::image::count_accessible(&db)
            .await
            .unwrap(),
        0
    );
...
async fn test_image_list_images() {
    ...
    for i in 0..5 {
        ...
        randimg_core::db::query::image::update_fields(
            &db,
            img.id,
            serde_json::json!({"is_public": true, "accessible": true}),
        )
        .await
        .unwrap();
    }
...
async fn test_image_list_images_sorted_by_width() {
    ...
    for (i, width) in [(100, 800), (200, 1200), (300, 640)].iter() {
        ...
        let img = randimg_core::db::query::image::create_image(&db, &data)
            .await
            .unwrap();
        randimg_core::db::query::image::update_fields(
            &db,
...
async fn test_image_list_images_by_author_filter() {
    let db = setup_db().await;
    let config = make_test_config();
    let a1 = randimg_core::db::query::author::find_or_create(&db, "Alice", Some("p"), Some("1"))
        .await
        .unwrap();
    let a2 = randimg_core::db::query::author::find_or_create(&db, "Bob", Some("p"), Some("2"))
        .await
        .unwrap();
...
async fn test_image_with_tags() {
    ...
    let tag1 = randimg_core::db::query::tag::find_or_create(&db, "nature", None)
        .await
        .unwrap();
    let tag2 = randimg_core::db::query::tag::find_or_create(&db, "sunset", Some("日落"))
        .await
        .unwrap();
...
async fn test_image_list_with_tag_filter() {
    ...
    let nature_tag = randimg_core::db::query::tag::find_or_create(&db, "nature", None)
        .await
        .unwrap();
...
fn derived_status_active_overrides_failed() {
    ...
    assert_eq!(
        derived_status_from_flags(true, true, false, true),
        "running"
    );
    assert_eq!(derived_status_from_flags(true, true, true, true), "running");
    assert_eq!(
        derived_status_from_flags(true, false, true, true),
        "running"
    );
    assert_eq!(
        derived_status_from_flags(true, false, false, false),
        "running"
    );
}
...
fn derived_status_terminal_outcomes() {
    ...
    assert_eq!(
        derived_status_from_flags(false, true, true, true),
        "partial_success"
    );
    // failed only, transient
    assert_eq!(
        derived_status_from_flags(false, true, false, false),
        "failed"
    );
    // completed only
    assert_eq!(
        derived_status_from_flags(false, false, true, false),
        "completed"
    );
    ...
    assert_eq!(
        derived_status_from_flags(false, false, false, false),
        "pending"
    );
}
...
fn derived_status_all_killed_is_terminal_killed() {
    ...
    assert_eq!(
        derived_status_from_flags(false, true, false, true),
        "killed"
    );
