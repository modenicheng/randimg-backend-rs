...
| 数据库 | SQLite / PostgreSQL（feature-gated），API 与队列分离 |
| 认证 | JWT + Argon2 |
| 图片处理 | `image` + Rayon |
...
# 默认使用 PostgreSQL 队列，确保 QUEUE_DATABASE_URL 可连
```

### 2. 构建与运行

```bash
# 默认：SQLite API + PostgreSQL 队列
cargo build
cargo run -p randimg-server

# 全 SQLite（单机开发，无需 PostgreSQL）
cargo build --no-default-features --features db-sqlite,queue-sqlite
cargo run -p randimg-server --no-default-features --features db-sqlite,queue-sqlite

# 生产环境：PostgreSQL API + PostgreSQL 队列
cargo build --no-default-features --features db-postgres,queue-postgres
cargo run -p randimg-server --no-default-features --features db-postgres,queue-postgres
```

> **注意**：`db-sqlite` 和 `db-postgres` 互斥，`queue-sqlite` 和 `queue-postgres` 互斥。
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `API_DATABASE_URL` | `sqlite://data/randimg.db?mode=rwc` | API 数据库连接串（SeaORM） |
| `QUEUE_DATABASE_URL` | — | 队列数据库连接串（Fang，PostgreSQL） |
