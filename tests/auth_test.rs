...
use randimg_backend_rs::auth::jwt::{Claims, create_token, verify_token};
use randimg_backend_rs::auth::password::{hash_password, verify_password};
...
fn test_create_and_verify_token_roundtrip() {
    ...
    let token = create_token("alice", secret, 60);
...
fn test_verify_token_wrong_secret_fails() {
    let token = create_token("alice", "correct-secret", 60);
...
fn test_create_token_different_users_different_tokens() {
    ...
    let token_a = create_token("alice", secret, 60);
    let token_b = create_token("bob", secret, 60);
...
fn test_create_token_contains_expected_claims() {
    let secret = "test-secret";
    let token = create_token("carol", secret, 30);
