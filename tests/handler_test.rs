...
async fn build_test_router(db: DatabaseConnection, config: AppConfig) -> axum::Router {
    use randimg_backend_rs::handlers::{health, auth, tag, author, statistic, image};

    let job_storage = JobStorage::new_for_test().await;
    // Create a separate Apalis pool for the state (in-memory, ephemeral)
    let apalis_pool = apalis_sqlite::SqlitePool::connect("sqlite::memory:")
        .await
...
async fn test_login_success() {
    ...
    let password_hash = randimg_backend_rs::auth::password::hash_password("testpassword");
    randimg_backend_rs::db::query::admin::create(&db, "admin", &password_hash, true)
        .await
        .unwrap();
...
async fn test_login_wrong_password() {
    ...
    let password_hash = randimg_backend_rs::auth::password::hash_password("correct");
    randimg_backend_rs::db::query::admin::create(&db, "admin", &password_hash, true)
        .await
        .unwrap();
...
async fn test_get_tags_with_data() {
    ...
    randimg_backend_rs::db::query::tag::find_or_create(&db, "landscape", Some("风景")).await.unwrap();
    randimg_backend_rs::db::query::tag::find_or_create(&db, "portrait", Some("肖像")).await.unwrap();
...
async fn test_get_tags_with_pagination() {
    ...
    for i in 0..5 {
        randimg_backend_rs::db::query::tag::find_or_create(&db, &format!("tag{}", i), None).await.unwrap();
    }
...
async fn test_update_tag_with_auth() {
    ...
    let tag = randimg_backend_rs::db::query::tag::find_or_create(&db, "sunset", None).await.unwrap();

    let pw_hash = randimg_backend_rs::auth::password::hash_password("pw");
    randimg_backend_rs::db::query::admin::create(&db, "admin", &pw_hash, true).await.unwrap();
    let token = randimg_backend_rs::auth::jwt::create_token("admin", &config.secret_key, 60);
...
async fn test_delete_tag_with_auth() {
    ...
    let tag = randimg_backend_rs::db::query::tag::find_or_create(&db, "deleteme", None).await.unwrap();
...
async fn test_list_authors_with_data() {
    ...
    randimg_backend_rs::db::query::author::find_or_create(&db, "Artist1", Some("pixiv"), Some("p1")).await.unwrap();
    randimg_backend_rs::db::query::author::find_or_create(&db, "Artist2", Some("twitter"), Some("t1")).await.unwrap();
...
async fn test_statistic_with_data() {
    ...
    randimg_backend_rs::db::query::tag::find_or_create(&db, "t1", None).await.unwrap();
    randimg_backend_rs::db::query::tag::find_or_create(&db, "t2", None).await.unwrap();
